...
const filterReports = data => {
    ...
    data.forEach(obj => {
        if(!obj.is_training) {
            ...
            const disaster_type = obj.disaster_type;
            const report_data = obj.report_data;
            const disasterSeverity = getDisasterSeverity(disaster_type, report_data);
            let existingRegion = transformedReportCounts.find(item => item.regionCode === regionCode && item.disasterType === disaster_type);
            if (existingRegion) {
                ...
                transformedReportCounts.push({ regionCode, count,  disasterType: disaster_type , city: city || ""});
            }
        }
        });
...
const handleResponse = (data, req, res) => {
    return !data ? res.status(404).json({ message: "Cards not found" }) : res.status(200).json({ result: data });
};
    ...
    return !data
        ...
        ? // If CAP format has been required convert to geojson then to CAP
          formatGeo(data, "geojson")
              .then(formatted => res.header("Content-Type", "text/xml").send(cap.geoJsonToReportAtomCap(formatted.features)))

              /* istanbul ignore next */
              .catch(err => console.log("🚀 ~ file: utils.js ~ line 77 ~ handleGeoCapResponse ~ err", err))
        : // Otherwise hand off to geo formatter
          formatGeo(data, req.query.geoformat)
              ...
              .catch(err => {
                  ...
                  return res.status(400).json({ statusCode: 400, error: "Error while formating" });
              });
    /* istanbul ignore next */
    // .catch((err) => next(err));
};
...
const checkIfPointInGeometry = (data, req, res) => {
    return !data
        ...
        : simplifyGeoAndCheckPoint(data, req.query.geoformat, req.query.lat, req.query.long)
              ...
              /* istanbul ignore next */
              .catch(err => {
                  console.log("🚀 ~ file: utils.js ~ line 129 ~ checkIfPointInGeometry ~ err", err);
                  res.status(400).json({ statusCode: 400, message: "Error while forming response" });
              });
};
