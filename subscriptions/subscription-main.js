...
app.delete('subscriptions/delete-subscriber', (req, res) => {
    if (!req?.body?.userId) {
        return res.status(400).json({ message: 'Bad Request , whatsapp number is needed' })
    }
    console.log('Coming inside delete method', req?.body?.userId)
    return subscriptions(config, db)
        .deleteSubscription(req?.body?.userId)
